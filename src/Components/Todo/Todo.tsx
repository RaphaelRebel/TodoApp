...
const Todo = () => {
  ...
  const [cardsStillAllowed, setCardsStillAllowed] = useState<TodoCardsProps[]>(TodoCards)

  const addTodoCard = () => {
    const newTodoId = (cardsStillAllowed[cardsStillAllowed.length - 1]?.id ?? 0) + 1;
  ...
  const removeCard: (id: any) => void = (id) => {
      const updatedTodos = cardsStillAllowed.filter((cardsStillAllowed) => cardsStillAllowed.id !== id);
  ...
  return (
    <section className="todo">
      ...
      <div className="todoCards">{todoCardsToBeRendered}</div>
    </section>
  );
};
